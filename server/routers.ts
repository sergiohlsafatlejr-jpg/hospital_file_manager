  ...
  arquivos: router({
    ...
    reprocessar: protectedProcedure
      ...
      .mutation(async ({ input, ctx }) => {
        ...
        // Resetar status
        await db.updateArquivoStatus(input.arquivoId, 'processando');
        await db.updateArquivoProgresso(input.arquivoId, 0, 0);
        
        // Baixar arquivo do S3 (usar s3Url direta)
        const downloadUrl = arquivo.s3Url;
        if (!downloadUrl) {
          await db.updateArquivoStatus(input.arquivoId, 'erro');
          throw new TRPCError({ code: "INTERNAL_SERVER_ERROR", message: "Arquivo sem URL no S3" });
        }
        const response = await fetch(downloadUrl);
        if (!response.ok) {
          await db.updateArquivoStatus(input.arquivoId, 'erro');
          throw new TRPCError({ code: "INTERNAL_SERVER_ERROR", message: "Falha ao baixar arquivo do S3" });
        }
        ...
        const processInBackground = async () => {
          try {
            if (arquivo.direcao === 'retornado' && arquivo.tipoArquivo === 'excel') {
              // Limpar dados antigos
              await db.deleteRecebimentosExcelByArquivo(input.arquivoId);
              await db.deleteDemonstrativoByArquivo(input.arquivoId);
              
              const { parseExcelRecebimentosExcelChunked } = await import('./recebimentosExcelParser');
              await db.updateArquivoProgresso(input.arquivoId, 10, 0);
              ...
              const result = await parseExcelRecebimentosExcelChunked(
                ...
                input.arquivoId,
                arquivo.convenioId,
                arquivo.dataReferencia ? new Date(arquivo.dataReferencia) : undefined,
                arquivo.dataPagamento ? new Date(arquivo.dataPagamento) : undefined,
                ...
                async (records, chunkIdx, totalRows) => {
                  ...
                  totalInserted += inserted;
                  const progresso = Math.round(10 + (totalInserted / totalRows) * 75);
                  await db.updateArquivoProgresso(input.arquivoId, Math.min(progresso, 85), totalInserted, totalRows);
                  console.log(`[Reprocessar] Chunk ${chunkIdx}: +${inserted} (total: ${totalInserted}/${totalRows})`);
                }
              );
              ...
              if (result.totalRecords > 0) {
                ...
                try {
                  const { syncDemonstrativoByArquivo } = await import('./syncDemonstrativo');
                  const syncResult = await syncDemonstrativoByArquivo(input.arquivoId, 'excel');
                  if (syncResult.success) {
                ...
                await db.updateArquivoProgresso(input.arquivoId, 95, result.totalRecords, result.totalRecords);
              }
              
              await db.updateArquivoStatus(input.arquivoId, 'processado');
              await db.updateArquivoProgresso(input.arquivoId, 100, totalInserted, result.totalRows);
              console.log(`[Reprocessar] Concluído: ${totalInserted} registros`);
            } else {
              await db.updateArquivoStatus(input.arquivoId, 'erro');
            }
          } catch (error) {
        ...
        processInBackground().catch(err => {
          console.error('[Reprocessar] Erro não tratado:', err);
          db.updateArquivoStatus(input.arquivoId, 'erro').catch(() => {});
        });
        
        return { success: true, message: 'Reprocessamento iniciado em background' };
      }),
